...
export default function ClientApp() {
  ...
  return (
    <div className="min-h-screen bg-[#080d18] flex flex-col max-w-md mx-auto relative">
      {/* Top bar */}
      <div className="flex items-center justify-between px-5 pt-safe-top pt-5 pb-4 border-b border-white/5 flex-shrink-0">
        <div>
          <p className="text-[9px] font-bold text-gray-600 uppercase tracking-widest">Portal do Cliente</p>
          <p className="text-sm font-black text-white">{cliente.nome}</p>
        </div>
      ...
      {/* Bottom nav */}
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-[#0a0f1a]/95 backdrop-blur-xl border-t border-white/8 px-4 pt-3 pb-safe-bottom pb-4 flex items-center justify-around z-50">
        {NAV.map(({ id, icon: Icon, label }) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center gap-1 px-3 py-1 rounded-none transition-all"
            >
              ...
              <span className={`text-[9px] font-black uppercase tracking-wider transition-all ${active ? 'text-emerald-400' : 'text-gray-700'}`}>
                {label}
              </span>
              {active && (
                <motion.div layoutId="nav-indicator" className="absolute bottom-3 w-5 h-0.5 bg-emerald-500 rounded-none" />
              )}
            </button>
          );
        })}
